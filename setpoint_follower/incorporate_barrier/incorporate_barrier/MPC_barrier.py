...
def optimize_path_first(horizon, x0, t0, squares: list[HyperCubeHandler], agents: list[int], dt, return_val):
    assert type(x0) == np.ndarray

    DIM = len(squares[0].compute_offset_vector(0))//2

    state_size = len(agents)*DIM
    input_size = len(agents)*DIM

    u = cp.Variable((input_size, horizon-1))
    slack = cp.Variable((input_size, horizon-1))
    x = cp.Variable((state_size, horizon))
    ...
    if b_pos:
        ...
        constraints = [x[:,0] == x0]
        constraints += [x[:,1:] == A_d @ x[:,:-1] + B_d @ (u+slack)]
        constraints +=  [u <= 0.3]
        constraints +=  [-u <= 0.3]
        ...
        objective = cp.Minimize(cp.sum_squares(u) + 1e3*cp.sum_squares(slack))
        prob = cp.Problem(objective, constraints)
        ...
        return np.zeros((len(agents)*DIM))
